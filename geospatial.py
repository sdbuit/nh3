...
class CoordinateMapper:
    ...
    def map_elevations_v2(self, vehicle_data: List[Coordinate], use_process_map=False) -> List[np.float32]:
        logging.info('Using process_map for altitude mapping...'
                     if use_process_map
                     else 'Using thread_map for altitude mapping...')
        points = [v.point for v in vehicle_data]
        if use_process_map:
            closest_coords = process_map(self.find_closest_point, points, chunksize=50)
        else:
...
def merge_altitudes(original_coords: List[Coordinate], altitudes: List[float]) -> List[Coordinate]:
    updated = []
    for old_coord, new_alt in zip(original_coords, altitudes):
        updated.append(Coordinate(
            point=Point(
...
def update_vehicle_data_with_elevation(
    vehicle_data: List[Coordinate],
...
def calculate_elevation_gain_pipeline(df: pl.DataFrame, window_size=5, 
    threshold=1.0) -> float:
