"""
Computes Vehific Specific (VSP) using physics-based approach with NumPy.
"""
import logging
...
class VSPCalculator:
    """
    Calculates VSP using vehicle parameters and driving data.
    """
    def __init__(
        self,
        vehicle_mass_kg: float,
        rolling_resistance_coeff: float,  # Unitless
        drag_coeff: float,               # Unitless
        frontal_area_m2: float,          # m^2
        ...
        logger.info(
            f'VSPCalculator initialized: mass={vehicle_mass_kg:.1f} kg, '
            f'Crr={rolling_resistance_coeff:.4f}, Cd={drag_coeff:.3f}, '
            f'A={frontal_area_m2:.2f} m^2, eta={drivetrain_efficiency:.2f}, '
            'rho={air_density_kg_m3:.3f} kg/m^3')
        logger.info(f'Calculated rolling_term: {self.rolling_term:.4f} m/s^2')
        logger.info(f'Calculated drag_term: {self.drag_term:.6f} 1/m')

    def _calculate_acceleration(self, time_s_np: np.ndarray, speed_mps_np: np.ndarray, smooth_window: Optional[int] = 3) -> np.ndarray:
        """
        Calculates acceleration from time and speed arrays, with optional smoothing.
        """
    ...
    def calculate_vsp(self, df: pl.DataFrame, smooth_speed_window: Optional[int] = 3) -> np.ndarray:
        """
        Calculates VSP (kW/tonne) from a Polars DataFrame using NumPy.
        Uses clearer variable names for extraction.
        """
        file_basename = 'VSP COMPUTE'
        ...
        try:
            logger.debug(f'[{file_basename}] Extracting data to NumPy arrays...')
            ...
            logger.debug(
                f'[{file_basename}] Extracted array shapes: '
                f'time={time_s_np.shape}, speed={speed_raw_np.shape}, grade={grade_percent_np.shape}')
        ...
        valid_mask = (
            np.isfinite(speed_mps_np) &
            np.isfinite(grade_decimal_np) &
            np.isfinite(acceleration_mps2_np))
        vsp_w_kg[valid_mask] = ((
            self.rolling_term
            + self.drag_term * np.square(speed_mps_np[valid_mask])
            + GRAVITY * grade_decimal_np[valid_mask]
            + acceleration_mps2_np[valid_mask])
            * speed_mps_np[valid_mask]
            / self.drivetrain_efficiency)
        vsp_kw_tonne = vsp_w_kg
